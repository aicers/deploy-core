...
use deploy_core::payload::{
    ArtifactInput, EnvelopeBlock, PayloadError, Signed, append_trailer_signed,
    read_package_container,
};
...
fn a_consumer_can_read_bounded_metadata_without_a_trust_set() {
    ...
    let container = read_package_container(Cursor::new(package), &ENVELOPE_BOUNDS)
