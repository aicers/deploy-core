...
#[test]
fn the_zstd_form_keeps_one_compatibility_path_per_layer_position() {
    let layer = json!({
        "compression": "gzip",
        "entries": [{ "kind": "file", "path": "a.txt", "text": "a\n" }],
    });
    let parameters = json!({
        "form": "zstd-layer",
        "base": {
            "architecture": "amd64",
            "layers": [layer, layer],
            "public_refs": ["registry.example/app:1"],
        },
    });
    let generated = raw::generate(&parameters);
    let entries = raw::read_entries(&generated.bytes);
    let compatibility: Value = serde_json::from_slice(
        entries
            .iter()
            .find(|entry| entry.name == COMPATIBILITY_FILE)
            .and_then(|entry| entry.data.as_deref())
            .unwrap(),
    )
    .unwrap();
    let layers = compatibility[0]["Layers"].as_array().unwrap();
    assert_eq!(layers.len(), 2);
    assert_eq!(layers[0], layers[1]);
    let blob = entries
        .iter()
        .filter(|entry| entry.name.starts_with("blobs/sha256/"))
        .filter(|entry| layers[0] == entry.name.as_str())
        .count();
    assert_eq!(blob, 1);
}
...
/// `cargo test --lib -- --ignored write_synthetic_fixtures` once generated
/// bytes move, such as under a newly resolved `flate2` or `miniz_oxide`
/// release (`Cargo.lock` is not tracked, so nothing pins them), and review
/// the diff. `docker-capture` fixtures are left untouched.
#[test]
