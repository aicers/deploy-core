...
- `payload::append_trailer_signed`, which gives a caller-supplied signer the exact manifest bytes the writer emits and stamps its detached Ed25519 signature and `key_id` into either a `.pkg` package or an installer payload. The signing key and its custody remain entirely with the caller.
